...
class FreshLeadsView extends GetView<FreshLeadsController> {
  const FreshLeadsView({super.key});
  @override
  Widget build(BuildContext context) {
    final FreshLeadsController controller = Get.put(FreshLeadsController());
    return Scaffold(
      extendBody: true,
      appBar: CustomAppBar(
        title: CustomSearchBar(
          ...
          hintText: 'search in Fresh leads ',
        ),
      ),
      body: controller.obx(
        onEmpty: const CustomEmptyScreen(label: 'No Leads Found'),
        onLoading: const CustomLoadingScreen(),
        (FreshLeadsView? state) => Column(
          children: <Widget>[
                    ...
                    children: <Widget>[
                      ...
                      Container(
                        ...
                        child: Center(
                          child: Obx(() {
                            return Text(
                              controller.freshLeads.length.toString(),
            ...
            Obx(
              () => controller.freshLeads.isNotEmpty
                  ? Expanded(
                          ...
                          child: Column(
                            children: <Widget>[
                              ListView.separated(
                                ...
                                itemCount: controller.freshLeads.length,
                                itemBuilder:
                  ...
                  : const Expanded(
                      child: Column(
                        mainAxisAlignment: MainAxisAlignment.center,
                        children: <Widget>[
                          CustomEmptyScreen(label: 'No leads Found'),
                        ],
                      ),
                    ),
            ),
  ...
  Widget buildSingleLead(int index) {
    return Padding(
      ...
      child: ListTile(
        dense: true,
        onTap: () => controller
            .onTapSingleLead(controller.freshLeads[index].leadId.toString()),
        title: Center(
          child: Text(
            controller.freshLeads[index].customerName.toString(),
            style: const TextStyle(
              fontWeight: FontWeight.w500,
            ),
          ),
        ...
        trailing: Text(
          controller.freshLeads[index].customerProgress.toString(),
          style: const TextStyle(
            fontWeight: FontWeight.w500,
          ),
        ),
